...
def formatar_saida(solucao, mapeamento_id, nome_instancia, clocks_ref=0, clocks_sol=0):
    ...
    custo_total = sum(r.custo_total for r in solucao)
    total_rotas = len(solucao)
    # Formata as linhas de rota
    linhas_rotas = []
    for idx, rota_obj in enumerate(solucao, 1):
        demanda = rota_obj.demanda_total
        custo_r = int(rota_obj.custo_total)
        detalhes = rota_obj.detalhes_visitas
        total_visitas = 2 + len(detalhes)  # depósito ida e volta + serviços
        linha = f" 0 1 {idx} {demanda} {custo_r}  {total_visitas} (D 0,1,1)"
        for s in detalhes:
            # (S id, u, v)
            linha += f" ({s[0]} {s[1]},{s[2]},{s[3]})"
        linha += " (D 0,1,1)"
